...
export class HomeService {
  ...
  getProductBestOutstanding(callback: (products) => void): void {
    const api = this.configService.get('APIs').product.bestOutstanding;
  ...
  getProductBestSelling(callback: (products) => void): void {
    const api = this.configService.get('APIs').product.bestSelling;
    this.loadingService.start();
    this.apiService.get(api).subscribe(
      res => {
        if (res.Status === this.apiStatus.success) {
